...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
     <title>AdKrive - Ads of  ${advertise.name}</title>
    <!-- site favicon -->
    <base
...
<!-- navbar-wrapper end -->
    ...
    <div class="col-lg-6 col-sm-6">
        <h6 class="page-title">Ads of  ${advertise.name}</h6>
    </div>
                        ...
                        <table class="table table--light style--two">
                            ...
                            <tbody>
									...
									<c:choose>
											...
											<c:when test="${empty advListAid}">
												 <td class="text-muted text-center" colspan="100%">No ads</td>
											</c:when>
											
											<c:otherwise>
													<c:forEach items="${advListAid}" var="element">
															
															<tr>
																<td data-label="Ad Name" class="text--primary">${element.ad_name}</td>
																<td data-label="Redirect Url" class="text--primary">${element.redirect_url}</td>
																<td data-label="Type" class="text--primary">${element.ad_type}</td>
																<td data-label="Status" class="text--primary">${element.status}</td>
																<td data-label="Action" class="text--primary">${element.status}</td>
																
																<td data-label="Action">
                                        <button type="button" class="icon-btn btn--primary edit"
                                                data-name="${element.adName}"
                                                data-Redirect Url="${element.redirect_url}"
                                                data-Type="${element.ad_type}"
                                                data-status="${element.status}"
                                                data-route="/advertise/update/ads/${element.id}">
