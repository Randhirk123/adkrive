...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
 <title>AdKrive - Email Templates</title>
<base
...
<body>
    ...
    <div class="col-lg-6 col-sm-6">
        <h6 class="page-title">EMAIL Templates</h6>
    </div>
                            ...
                            <c:choose>
                            			<c:when test="${empty emailList}">
													<td class="text-muted text-center" colspan="100%">No
														Email Template found</td>
												</c:when>
                            			<c:otherwise>
                            				<c:forEach items="${emailList}" var="element">
                            				
                            				  <tr>
                                    <td data-label="Name">${element.name}</td>
									 <td data-label="Subject">${element.subj}</td>
                                    <td data-label="Status">
                                    <c:if test="${element.email_status eq 1}"><span class="text--small badge font-weight-normal badge--success">Active</span></c:if>  
                                           <c:if test="${element.email_status eq 0}"><span class="text--small badge font-weight-normal badge--warning">Disable</span></c:if>  </td>
                                                                           

                                    <td data-label="Action">
                                        <a href="/email-template/${element.id}/edit"
